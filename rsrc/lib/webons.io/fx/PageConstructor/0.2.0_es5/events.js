...
*/ ///***************************************************************************>
(function (fx) {
  var $ = new fx(fx.vbind('lib://webons.io/fx/PageConstructor/') + 'events');$.prv = { $: { lid: 0, reg: {} }, events: function () {
      function events($rid, $eid) {
        _classCallCheck(this, events);

        var $lid = 0;this.$ = { get rid() {
            return $rid;
          }, get eid() {
            return $eid;
          }, get lid() {
            $lid = $lid + 1;return $lid;
          } };this.name = name;this.callbacks = {};
      }
      ...
      function generator() {
        ...
        if ($.prv.$.reg[0] && $.prv.$.reg[0].settings.singleton) {
          ...
          var $id = $.prv.$.lid;$.prv.$.lid++;$.prv.$.reg[$id] = { events: {}, settings: $settings };this.$ = { get id() {
              return $id;
            }, get settings() {
              return $settings;
            } };
      ...
      _createClass(generator, [{
        ...
        value: function register(eid) {
          ...
          }var args = Array.prototype.slice.call(arguments);args = args.slice(3);if (node && auto) {
            var _ret = function () {
              if (auto === true) {
                auto = eid;
              }var tmp = _this,
                  task = void 0;if (!$.fx.isTypeOf(node, NodeList)) {
                task = function task() {
                  tmp.dispatch.apply(tmp, [eid, node].concat(Array.prototype.slice.call(arguments), _toConsumableArray(args)));
                };node.addEventListener(auto, task);return {
                  v: {
                    remove: function remove() {
                ...
                var _loop = function _loop(i) {
                  ...
                  };node[i].addEventListener(auto, task);result.push({
                    remove: function remove() {
        ...
        value: function dispatch(eid, tArg) {
          if ($.prv.$.reg[this.$.id].events[eid]) {
            var $rid = this.$.id,
                args = Array.prototype.slice.call(arguments);args = args.slice(2);Object.keys($.prv.$.reg[this.$.id].events[eid].callbacks).forEach(function (key) {
